...
class UI_Window(object):
    ...
    def checkArrowStatus(func):
        def wrapper(self, *args):
            ...
            if self.status == 0:
                ...
                if self.teamType is not None:
                    self.rightButton.setEnabled(True)
            elif self.status == 2:
                self.leftButton.setEnabled(True)

            if self.gameType == "Other":
                self.mmrBalancingButton.setVisible(False)
            else:
                self.mmrBalancingButton.setVisible(True)
        return wrapper

    @checkArrowStatus
    def leftArrowClick(self, *args):
        self.status = max(0, self.status - 1)
        self.variableMenu.setCurrentIndex(self.status)
        self.mainWindow.setCurrentIndex(self.status)

    @checkArrowStatus
    def rightArrowClick(self, *args):
    ...
    def randomTeamButtonClick(self, *args):
        ...
        ATeam, BTeam = self.userList[:self.userNum // 2], self.userList[self.userNum // 2:]

        self.teamType = "random"
    ...
    def setupUI(self, window):
        ...
        window.setWindowTitle("TeamBalancer")

        window.setObjectName("baseWindow") # ObjectName is required for the StyleSheet
        ...
        self.a_lol.clicked.connect(lambda : self.gameButtonClick("League of Legends"))
        self.b_siege.clicked.connect(lambda : self.gameButtonClick("Siege"))
        self.c_other.clicked.connect(lambda : self.gameButtonClick("Other"))
        ...
        # ATeamMember List
        self.ATeamMember = QtWidgets.QWidget(self.ATeamPage)
        ...
        self.ATeamMember.setSizePolicy(sizePolicy)
        self.ATeamMemberVerticalLayout = QtWidgets.QVBoxLayout(self.ATeamMember)

        self.ATeamVerticalLayout.addWidget(self.ATeamMember)
        ...
        # BTeamMember List
        self.BTeamMember = QtWidgets.QWidget(self.BTeamPage)
        ...
        self.BTeamMember.setSizePolicy(sizePolicy)
        self.BTeamMemberVerticalLayout = QtWidgets.QVBoxLayout(self.BTeamMember)

        self.BTeamVerticalLayout.addWidget(self.BTeamMember)
